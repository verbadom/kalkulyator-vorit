...
calculateBtn.addEventListener("click", async () => {
  ...
  html += `
    <div class="result-actions">
      <a href="tel:+380673990560" class="btn-call">📞 Подзвонити</a>
      <div class="phone-number-text">+38 (067) 399-05-60</div>

      <button class="btn-messenger" id="messengerToggleBtn">✉️ Написати в месенджер</button>
      <div class="messenger-choice" id="messengerChoice" style="display:none;">
        <a href="viber://chat?number=%2B380673990560&text=${encodeURIComponent('Вітаю! Я порахував вартість воріт у калькуляторі. Хочу уточнити деталі - маю кілька питань.')}" class="msg-btn viber" data-messenger="Viber">Viber</a>
        <a href="https://t.me/+380673990560?text=${encodeURIComponent('Вітаю! Я порахував вартість воріт у калькуляторі. Хочу уточнити деталі - маю кілька питань.')}" class="msg-btn telegram" data-messenger="Telegram">Telegram</a>
        <a href="https://wa.me/380673990560?text=${encodeURIComponent('Вітаю! Я порахував вартість воріт у калькуляторі. Хочу уточнити деталі - маю кілька питань.')}" class="msg-btn whatsapp" data-messenger="WhatsApp">WhatsApp</a>
      </div>
...
function showResult(html, showShareBtns) {
  ...
  // Кнопка "Написати в месенджер" — toggle вибору
  const messengerToggleBtn = document.getElementById("messengerToggleBtn");
  const messengerChoice = document.getElementById("messengerChoice");
  if (messengerToggleBtn && messengerChoice) {
    messengerToggleBtn.addEventListener("click", function() {
      const isVisible = messengerChoice.style.display !== "none";
      messengerChoice.style.display = isVisible ? "none" : "flex";
    });
  }

  // Трекінг кліків по месенджерам
  ...
  if (shareBtn) {
    shareBtn.addEventListener("click", async function() {
      const shareUrl = "https://verbadom.github.io/kalkulyator-vorit/";
      const shareText = "Я порахував тут собі ворота — доволі зручно, глянь, може тобі теж актуально 👇";
      if (navigator.share) {
        try {
          await navigator.share({ title: "Калькулятор воріт — Verbadom", text: shareText, url: shareUrl });
          track('share', selectedCityName);
        } catch(e) {}
      } else {
        try { await navigator.clipboard.writeText(shareUrl + "\n" + shareText); alert("Посилання скопійовано!"); } catch(e) {}
      }
    });
  }
  ...
  if (showShareBtns) {
    ...
    const newCalcBtn = document.createElement("button");
    newCalcBtn.className = "btn-new-calc";
    newCalcBtn.innerHTML = "🔄 Почати новий розрахунок";
    newCalcBtn.addEventListener("click", () => {
      document.getElementById("resetBtn").click();
      window.scrollTo({ top: 0, behavior: "smooth" });
    });
    resultDiv.appendChild(newCalcBtn);

    // Ссылка в каталог
    const catalogLink = document.createElement("a");
    catalogLink.href = "https://verbadom.com.ua/ua/g140836156-vorota-kalitkoj-raspashnye";
    catalogLink.className = "catalog-link";
    catalogLink.textContent = "← Повернутися до каталогу воріт";
    catalogLink.target = "_blank";
    resultDiv.appendChild(catalogLink);
  }

  resultDiv.scrollIntoView({ behavior: "smooth" });
  document.getElementById("resetBtn").style.display = "none"; // прячем старую кнопку reset снаружи
}
